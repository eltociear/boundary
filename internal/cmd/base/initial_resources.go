...
func (b *Server) createPostgresAliasTarget(ctx context.Context,
	...
	// Extract the address, port, and dbname of the database URL for use with this target
	idx := strings.Index(b.DatabaseUrl, "@")
	dbUrl := b.DatabaseUrl[idx+1:]
	idx2 := strings.Index(dbUrl, ":")
	idx3 := strings.Index(dbUrl, "/")
	idx4 := strings.Index(dbUrl, "?")
	portStr := dbUrl[idx2+1 : idx3]
	addr := dbUrl[:idx2]
	dbname := dbUrl[idx3+1 : idx4]
